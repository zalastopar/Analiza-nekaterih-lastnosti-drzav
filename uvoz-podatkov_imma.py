...
def oceni(page_content):
    ...
    # preverimo, če je na dražbi
        ...
        slovar['Type'] = 'Min bid'
        slovar['Price'] = spremeni_v_stevilko(bid.text)
        return slovar
...
def save_as_csv(filename, sez):
    colnames = list(set().union(*(d.keys() for d in sez)))  #https://stackoverflow.com/questions/11399384/extract-all-keys-from-a-list-of-dictionaries
    colnames = sorted([el for el in colnames if el not in ('Name', 'Type', 'Price')])
    colnames = ['Name'] + colnames + ['Type', 'Price']    
    

    with open(filename, 'w', encoding='utf-8') as csvfile: 
        writer = csv.DictWriter(csvfile, fieldnames = colnames) 
        writer.writeheader() 
        writer.writerows(sez) 
...
def main(redownload=True, reparse=True):
    ...
    for i in range(8940, 10000):
        ...
        # združimo lastnosti in ceno
        slovar.update(slo)

        # dodamo slovar vsem ostalim
        slovarji.append(slovar)

        print(str(i))
    
    # shranimo v csv datoteko
    save_as_csv(csv_filename, slovarji)
